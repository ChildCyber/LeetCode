...
func firstMissingPositive(nums []int) int {
	// 思路：利用数组下标本身来记录哪些数字出现过
	n := len(nums)

	// 原地置换：遍历数组，把每个在 [1, n] 范围内的数放到正确的位置
	for i := 0; i < n; {
		// nums[i] 应该放到 nums[nums[i]-1] 上
		// 条件：nums[i] 在 [1,n] 范围内，并且当前位置不是正确位置，避免越界，同时避免死循环（处理重复数时）
	...
	// 第二次遍历，找到第一个 nums[i] != i+1 的下标，答案就是 i+1
	for i := 0; i < n; i++ {
		if nums[i] != i+1 {
			return i + 1
		}
