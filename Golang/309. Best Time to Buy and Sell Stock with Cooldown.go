...
func maxProfit309(prices []int) int {
	...
	// buy[i] 代表第 i 天通过 buy 或者 cooldown 结束此天能获得的最大收益
	// sell[i] 代表第 i 天通过 sell 或者 cooldown 结束此天能获得的最大收益
	// price[i-1] 代表第 i 天的股票价格(由于 price 是从 0 开始的)
	...
	for i := 2; i < len(prices); i++ {
		...
		buy[i] = max(buy[i-1] /*cooldown*/, sell[i-2]-prices[i] /*buy*/) // 第 i 天选择 buy 的时候，要从 i-2 的状态转移，而不是 i-1
	}
