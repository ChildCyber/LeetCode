...
/* 快慢指针
   ...
   Floyd 判圈算法，检测链表是否有环
   思路：对 nums 数组建图，每个位置 i 连一条 i→nums[i] 的边。由于存在的重复的数字 target，因此 target 这个位置一定有起码两条指向它的边，因此整张图一定存在环，且要找到的 target 就是这个环的入口
*/
func findDuplicate(nums []int) int {
	// 先设置慢指针 slow 和快指针 fast，慢指针每次走一步，快指针每次走两步，两个指针在有环的情况下一定会相遇，此时再将 slow 放置起点 0，两个指针每次同时移动一步，相遇的点就是答案。
	// 快指针一次走 2 步，慢指针一次走 1 步，相交以后，快指针从头开始，每次走一步，再次遇⻅的时候就是成环的交点处，也即是重复数字所在的地方。
	slow := nums[0]
	fast := nums[nums[0]]
	for fast != slow {
