...
// 暴力
// 思路：枚举子串的起点i，然后往后扩展j，判断 [i..j] 是否包含了t的所有字符
...
func minWindow(s string, t string) string {
	...
	// 不断扩展右指针，直到窗口包含了t的所有字符，然后收缩左指针，尽量缩小窗口但仍保持包含所有字符
	// 记录最小的满足条件的窗口
	if len(t) == 0 || len(s) < len(t) {
		return ""
	}
	...
	for right < len(s) {
		c := s[right]
		right++ // 窗口的右边界始终指向下一个待处理的元素，窗口区间 [left, right)
		...
		// 5. 当窗口满足条件时，尝试收缩左边界
		for valid == len(need) { // 说明窗口已经覆盖t，开始收缩左指针，更新最优解
			...
			d := s[left]
			left++
			// 只有当被移出的字符d是t中需要的字符时，才需要去维护window和valid
			if _, ok := need[d]; ok {
				// 如果在移除之前window[d]刚好等于need[d]，说明窗口对字符d的需求是“恰好满足”的
				if window[d] == need[d] {
					valid-- // 一旦移除一个d，其频次就会变小，导致不再满足需求，于是valid--
				}
				window[d]--
			}
		}
